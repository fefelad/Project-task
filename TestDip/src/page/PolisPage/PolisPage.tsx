...
const policyBlocks: PolicyBlock[] = [
  ...
  {
    id: 'processing-order',
    title: '7. Порядок сбора, хранения, передачи и других видов обработки персональных данных',
    intro: [
    ...
    points: [
      ...
      '7.3. В случае выявления неточностей в персональных данных, Пользователь может актуализировать их самостоятельно, путем направления Оператору уведомление на адрес электронной почты Оператора Chernila@gmail.com с пометкой «Актуализация персональных данных».',
  ...
  {
    id: 'actions',
    title: '8. Перечень действий, производимых Оператором с полученными персональными данными',
    points: [
  ...
  {
    id: 'confidentiality',
    title: '10. Конфиденциальность персональных данных',
    intro: [
  ...
  },
  {
    ...
    points: [
      '11.1. Пользователь может получить любые разъяснения по интересующим вопросам, касающимся обработки его персональных данных, обратившись к Оператору с помощью электронной почты Chernila@gmail.com.',
...
function PolicyText({ children, className }: { children: string; className: string }) {
  return (
    <Text fontFamily="onest" className={className}>
      {children}
    </Text>
  );
}
...
export default function PolisPage() {
  return (
    <main className={style.personalDataPolicyPage}>
      <div className={style.container}>
        <Text fontFamily="involve" className={style.title}>
          Политика в отношении обработки персональных данных
        </Text>

        <div className={style.content}>
          {policyBlocks.map((block) => (
            <section className={style.section} key={block.id}>
              <Text fontFamily="involve" className={style.sectionTitle}>
                {block.title}
              </Text>

              {block.intro?.map((paragraph, index) => (
                <PolicyText className={style.intro} key={`${block.id}-intro-${index}`}>
                  {paragraph}
                </PolicyText>
              ))}

              {block.points?.map((point, index) => (
                <PolicyText className={style.point} key={`${block.id}-point-${index}`}>
                  {point}
                </PolicyText>
              ))}
            </section>
