  ...
  const teachers = [
    {
      ...
      image: ImgAlexsandra,
      description: 'Опытный практикующий дизайнер, который научит вас не только теории, но и реальным инструментам индустрии. Её подход сочетает развитие творческого мышления с точным пониманием пользовательского опыта. На занятиях царит вдохновляющая атмосфера, где рождаются смелые проекты. Вы получите не просто знания, а готовое портфолио для старта карьеры'
    },
    {
      id: 'mark-belov',
      name: 'Марк Белов',
      image: ImgMark,
      description: 'Преподаватель, который помогает превратить креативные идеи в рабочие дизайн-решения и сильное портфолио. Его уроки — это погружение в актуальные тренды и отточенное владение современными программами. Каждое занятие — это шаг к созданию проекта, которым не стыдно похвастаться. Идеальный наставник для тех, кто ценит структуру и конкретный результат.'
    },
    {
      id: 'vera-shilina',
      name: 'Вера Шилина',
      image: ImgVera,
      description: 'Преподаватель, вдохновляющий на смелые эксперименты и поиск уникального визуального языка. Он верит, что лучший дизайн рождается на стыке эстетики, функциональности и глубокого понимания аудитории. Её занятия — это творческая лаборатория, где вы научитесь  творить и критически оценивать свою работу.'
    }
  ];
